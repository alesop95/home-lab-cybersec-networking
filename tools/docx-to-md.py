...
MACRO_NAMES = {
    1: "01-introduzione",
    2: "02-ftth-fastweb",
    3: "03-spunti-di-sviluppo",
    4: "04-concetti-generali",
    5: "05-analisi-del-caso",
}
...
def main():
    ...
    # riferimenti non desiderati (qui: l'anonimizzazione), preservando l'analisi tecnica. Le regole
    ...
    def redigi(testo):
        """Applica le redazioni a una stringa, restituendo il testo e il numero di sostituzioni.

        Serve anche sui titoli, e non solo sul corpo del documento: dal titolo discendono lo
        slug del file e il nome della cartella, quindi un valore reale lasciato li' finirebbe
        nel percorso di un file tracciato, dove nessuna redazione del corpo lo raggiungerebbe.
        """
        n_tot = 0
        for rx, repl in redaction_rules:
            testo, n = rx.subn(repl, testo)
            n_tot += n
        return testo, n_tot
    ...
    root_file = get_file(root_index_path, 2, "# Documentazione generata dal documento sorgente", is_index=True)
    ...
    title_redactions = 0  # sostituzioni applicate ai titoli (e quindi a slug e cartelle)

    for idx, b in enumerate(blocks):
        if b["kind"] == "heading":
            level = b["level"]
            title_text, _n_red_titolo = redigi(b["md"].strip())
            title_redactions += _n_red_titolo
            if level == 2:
    ...
    report.append("Sostituzioni deterministiche (sidecar `tools/redactions.json`) per neutralizzare "
                  "i dati identificativi reali secondo `.claude/rules/anonymization.md`, preservando "
                  "l'analisi tecnica. Questa e' una "
                  "divergenza voluta dal testo verbatim del sorgente.")
    report.append("")
    report.append("- Totale sostituzioni: %d (di cui %d su titoli, quindi anche su slug e nomi di cartella)"
                  % (total_redactions + title_redactions, title_redactions))
    if redacted_files:
